...
local buffer, register, size, tbl, window = require("snap.common.buffer"), require("snap.common.register"),
    require("snap.view.size"), require("snap.common.tbl"), require("snap.common.window")
do end
...
local function layout(config)
    ...
    local title = _let_1_["title"] or "The preview"
    ...
    return {
        width = (width - col_offset - size.padding - size.padding - size.border),
        height = height0,
        row = (row + tbl.sum(tbl.take(sizes, index)) + border + padding),
        col = (col + col_offset + (size.border * 2) + size.padding),
        title = title,
        focusable = false
    }
end
local function create(config)
    ...
    local function update(view)
        if vim.api.nvim_win_is_valid(winnr) then
            ...
            do end
            (view)["height"] = layout_config0.height
