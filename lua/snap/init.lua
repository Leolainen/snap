...
local buffer, config, create_producer, input, register, request, results, tbl, view, window, _ =
    require("snap.common.buffer"), require("snap.config"), require("snap.producer.create"), require("snap.view.input"),
    require("snap.common.register"), require("snap.producer.request"), require("snap.view.results"),
    require("snap.common.tbl"), require("snap.view.view"), require("snap.common.window"), nil
...
local function map(key, run, opts)
    ...
    do
        ...
        if (_1_ == "string") then
            print(
                "[Snap API] The third argument to snap.map is now a table, treating passed string as command, this will be deprecated")
            command = opts
...
local function run(config1)
    ...
    assert((type(get_producer(config1.producer)) == "function"),
        "snap.run 'producer' must be a function or a table with a default function")
    ...
    if config1.hide_views then
        assert(vim.tbl_contains({ "boolean", "function" }, type(config1.hide_views)),
            "snap.run 'hide_views' must be a boolean or a function")
    else
    end
    ...
    if config1.mappings then
        assert((type(config1.mappings) == "table"), "snap.run 'mappings' must be a table")
    else
    end
    if config1.on_update then
        assert((type(config1.on_update) == "function"), "snap.run 'on_update' must be a function")
    else
    end
    local last_results = {}
    ...
    local function get_hide_views()
        ...
        elseif (config1.hide_views ~= nil) then
            local _35_ = type(config1.hide_views)
            if (_35_ == "function") then
                return config1.hide_views()
            elseif (_35_ == "boolean") then
                return config1.hide_views
            else
                return nil
            end
    ...
    local function on_exit()
        ...
        config1["views"] = nil
        for _0, _38_ in ipairs(views) do
            local _each_39_ = _38_
            local view0 = _each_39_["view"]
            view0:delete()
        end
        results_view:delete()
        input_view:delete()
    ...
    local function create_views()
        if has_views() then
            for index, producer in ipairs(config1.views) do
                local view0 = {
                    view = view.create({ layout = layout, index = index, ["total-views"] = total_views }),
                    producer =
                        producer
                }
    ...
    do
        local body_2_auto
        local function _42_(selection)
            for _0, _43_ in ipairs(views) do
                local _each_44_ = _43_
                local _each_45_ = _each_44_["view"]
                local bufnr = _each_45_["bufnr"]
                local winnr = _each_45_["winnr"]
                local width = _each_45_["width"]
                local height = _each_45_["height"]
                local producer = _each_44_["producer"]
    ...
    do
        local body_2_auto
        local function _49_(results0, force_views)
            if not exit then
                do
                    ...
                    if (result_size == 0) then
                        ...
                        for row in pairs(partial_results) do
                            ...
                            if has_meta(result, "positions") then
                                local function _52_()
                                    local _51_ = type(result.positions)
                                    if (_51_ == "table") then
                                        return result.positions
                                    elseif (_51_ == "function") then
                                        return result:positions()
                                    elseif true then
                                        local _0 = _51_
                                ...
                                buffer["add-positions-highlight"](results_view.bufnr, row, _52_())
                            else
                            end
                ...
                if (has_views() and (force_views or (tostring(last_requested_selection) ~= tostring(selection)))) then
                    ...
                    for _0, _57_ in ipairs(views) do
                        local _each_58_ = _57_
                        local view0 = _each_58_["view"]
    ...
    local function on_update(filter)
        ...
        config2["on-end"] = function()
            ...
            elseif has_meta(tbl.first(results0), "score") then
                ...
                tbl["partial-quicksort"](results0, 1, #results0, (results_view.height + cursor_row), _71_)
                last_results = results0
                write_results(last_results)
            else
            end
    ...
    local function on_next_item()
        ...
        return on_key_direction(_85_)
    end
    local function on_prev_page()
        local function _86_(_241)
            return (_241 - results_view.height)
        end
        return on_key_direction(_86_)
    end
    local function on_next_page()
        local function _87_(_241)
            return (_241 + results_view.height)
        end
        return on_key_direction(_87_)
    end
    local function set_next_view_row(next_index)
        if has_views() then
            local _local_88_ = tbl.first(views)
            local _local_89_ = _local_88_["view"]
            local winnr = _local_89_["winnr"]
            local bufnr = _local_89_["bufnr"]
            local height = _local_89_["height"]
            local line_count = vim.api.nvim_buf_line_count(bufnr)
            local _let_90_ = vim.api.nvim_win_get_cursor(winnr)
    ...
    local function on_next()
        if (config1.next or (config1.steps and (#config1.steps > 0))) then
            ...
            local function _96_()
                do
                    local _97_ = type(next)
                    if (_97_ == "function") then
                        local function _98_()
                            return results0
                        end
                        next_config["producer"] = next(_98_)
                    elseif (_97_ == "table") then
                        for key, value in pairs(next.config) do
                            next_config[key] = value
                        end
                        local _99_
                        if next.format then
                            _99_ = next.consumer(next.format(results0))
                        else
                            local function _100_()
                                return results0
                            end
                            _99_ = next.consumer(_100_)
                        end
                        next_config["producer"] = _99_
                    else
                    end
                end
    ...
    input_view = input.create({
        reverse = config1.reverse,
        mappings = config1.mappings,
        ["has-views"] = has_views,
        layout =
            layout,
        prompt = prompt,
        ...
        ["on-prev-item"] =
            on_prev_item,
        ["on-next-item"] = on_next_item,
        ["on-prev-page"] = on_prev_page,
        ["on-next-page"] = on_next_page,
        ["on-viewpageup"] = on_viewpageup,
        ["on-viewpagedown"] = on_viewpagedown,
        ["on-view-toggle-hide"] =
            on_view_toggle_hide,
        ["on-select-toggle"] = on_select_toggle,
        ["on-select-all-toggle"] = on_select_all_toggle,
        ["on-update"] = on_update
    })
